...
def open_file():
    ...
    status.update()
    try:
        image = ResizeableImage.ResizeableImage(filename)
    except:
...
def update_display():
    ...
    image.save_ppm(temp)
    photo = tk.PhotoImage(master=root, file=temp)
    display['image'] = photo
...
root = tk.Tk()
root.title('6.006 Seam Carving GUI')
status = tk.Label(text='Please open an image.')
...
show_button = tk.Button(buttons, text='Show Seam', command=show_seam)
show_button.pack(side='top', fill='x')
remove_button = tk.Button(buttons, text='Remove Seam', command=remove_seam)
remove_button.pack(side='top', fill='x')
multiple_frame = tk.Frame(buttons)
multiple_label = tk.Label(multiple_frame, text='Repeat:')
multiple_label.pack(side='left')
multiple_spin = tk.Spinbox(multiple_frame,
